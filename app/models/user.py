...
class User(UserMixin, db.Model):
    ...
    # Novos campos adicionados nas migrações
    country = db.Column(db.String(100), default='Portugal')
    timezone = db.Column(db.String(50), default='Europe/Lisbon')
    postal_code = db.Column(db.String(20))
    
    # Timestamps (atualizados conforme migração)
    ...
    def update_location_coordinates(self, latitude: float, longitude: float):
        """
        Atualiza coordenadas do usuário e sincroniza com sistema meteorológico
        ...
        # Sincronizar com sistema meteorológico
        try:
            from app.services.location_manager import LocationManager
            LocationManager.ensure_user_location(self)
        except Exception as e:
            import logging
            logging.getLogger(__name__).error(f"Erro ao sincronizar localização: {e}")
