...
class WeatherData(db.Model):
    ...
    @staticmethod
    def create_from_api_data(api_data, location_data, forecast_data=None):
        """
        Cria um novo registro WeatherData a partir de dados da API
        
        Args:
            api_data: Dados da API OpenWeatherMap
            location_data: Dados da localização (name, latitude, longitude)
            forecast_data: Dados de previsão (opcional)
            
        Returns:
            Nova instância de WeatherData
        """
        import json
        ...
        weather_record = WeatherData(
            ...
            api_timestamp=datetime.fromtimestamp(api_data.get('dt', 0), tz=timezone.utc) if api_data.get('dt') else None,
            ...
            # Condições climáticas
            condition=weather_data.get('description', 'Desconhecido'),
            condition_code=weather_data.get('id'),
